...
def signify(x): return "+" + str(x) if x > 0 else x
...
async def update_leaderboard(self):
    ...
    text1 = tabulate(table1, headers="firstrow",
    ...
    for i in range(len(tw)):
        ...
        table2.append([ordinal(i + 1), user.display_name,
                      signify(tw[i]["weekly"])])

    text2 = tabulate(table2, headers="firstrow",
                     tablefmt="github", numalign="left")
    ...
    for i in range(len(ac)):
        ...
        percent = 0 if ac[i]["hitrate"][1] == 0 else ac[i]["hitrate"][0] / \
            ac[i]["hitrate"][1]
    ...
    text3 = tabulate(table3, headers="firstrow",
                     tablefmt="github", numalign="left")
